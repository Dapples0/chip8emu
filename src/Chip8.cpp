...
void Chip8::emulateCycle(Display &display) {
    ...
    pc += 2;
    executeOperation(vX, vY, nnn, nn, n, display);
    // Do delays here
    if (delay_timer > 0) {
        --delay_timer;
    }
...
void Chip8::executeOperation(uint8_t vX, uint8_t vY, uint8_t n, uint8_t nn, uint8_t nnn, Display &display) {
        ...
        case 0x0000:
            switch (opcode & 0x000Fu) {
                case 0x0000: // 00E0 - Clears the screen
                    clearDisplay();
                break;
                case 0x000E: // Returns from subrotine
                    --sp;
                    pc = stack[sp];
                break;
            }
        ...
        case 0xD000: // DXYN - Draws a sprite at (vX, vY) with a width of 8 pixels and height of N pixels. VF is set to 1 if any screen pixels are flipped from set to unset when the sprite is drawn, and to 0 if that does not happen
            ...
            for (int row = 0; row < n; ++row) {
                ...
                for (int col = 0; col < 8; ++col) {
                    uint8_t sprite = memory[I + row] & (0x80u >> col);
                    uint32_t *gfxPixel = &gfx[(yPos + row) * WIDTH + (xPos + col)];

                    if (sprite) {
                        if (*gfxPixel == 0xFFFFFFFF) {
                            registerV[MAX_SIZE - 1] = 1;
                        }
